...
| --- | --- |
| **curl / PowerShell** | Node 24.15.0+, git, `gh`. The installer resolves the newest release, checks it against that release's checksum and hands it to npm — it never installs Node and never asks for sudo. |
| **npm** | Node 24.15.0+, git, `gh` |
...
| **Yarn (Bash/zsh)** | Yarn **Classic** *and* Node 24.15.0+, git, `gh`. Yarn 2 removed global installs, so modern Yarn cannot install a CLI at all |
