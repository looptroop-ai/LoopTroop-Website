...
| --- | --- |
| **curl / PowerShell** | Node 24.11.0+, git, `gh`. The installer resolves the newest release, checks it against that release's checksum and hands it to npm — it never installs Node and never asks for sudo. |
| **npm** | Node 24.11.0+, git, `gh` |
...
| **bun** | bun *and* Node 24.11.0+ (the launcher is a Node program), git, `gh` |
| **pnpm** | pnpm *and* Node 24.11.0+, git, `gh`. pnpm will not resolve a tag to a version published in the last 24 hours |
| **Yarn (Bash/zsh)** | Yarn **Classic** *and* Node 24.11.0+, git, `gh`. Yarn 2 removed global installs, so modern Yarn cannot install a CLI at all |
