...
Both artifacts carry ordered `workspace_probes` entries with required `id`, `command`, and `purpose`. Their `git_hooks` object contains a strict policy enum (`observe_only`, `validate_advisory`, `validate_required`, or `use_native_hooks`), read-only detected-hook records (`name`, normalized `path`, `source`, `executable`, optional `manager_hint`), and ordered validation commands (`id`, `hook`, `command`, `purpose`). The backend replaces any generated, raw-edited, or structured-edited policy with the ticket-start snapshot of the project policy; validation commands remain editable. An empty validation-command list is valid and preserved exactly, and the parser never synthesizes a command for an unknown hook.
