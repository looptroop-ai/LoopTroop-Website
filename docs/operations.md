...
- **Port fallback:** if the default OpenCode port (`4096`) is occupied by another process or rejects the configured credentials, `npm run dev` scans for the next free port and starts OpenCode there instead.
...
- **Ephemeral auth:** if neither `OPENCODE_PASSWORD` nor `OPENCODE_SERVER_PASSWORD` contains a nonblank value and a managed OpenCode server is about to start, LoopTroop generates a random password and shares it with the server and backend. A nonblank password is preserved. When run directly, `npm run dev:opencode` prints a newly generated password for manual sharing and never prints configured passwords. v2 uses the fixed username `opencode` and prefers `OPENCODE_PASSWORD`; v1 uses `OPENCODE_SERVER_USERNAME` and `OPENCODE_SERVER_PASSWORD`.
...
| `OPENCODE_PASSWORD` | v2 Basic auth password; used when nonblank and passed exactly as provided |
| `OPENCODE_SERVER_USERNAME` | v1 Basic auth username; defaults to `opencode`. v2 always uses `opencode` |
| `OPENCODE_SERVER_PASSWORD` | v1 Basic auth password and v2 fallback when `OPENCODE_PASSWORD` is blank or unset; managed children get an ephemeral password only when both variables are blank or unset |
