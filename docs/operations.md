...
| `LOOPTROOP_OPENCODE_LOGS=all` | Direct watcher fallback for `npm run dev:opencode`; selects supported logging flags from the resolved CLI's `serve --help` output when it launches managed OpenCode |
