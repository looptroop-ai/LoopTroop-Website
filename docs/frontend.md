...
Opening a `DropdownPicker` moves focus into its popup, and closing it hands focus back to the control that opened it — tab order follows document order, and a popup portaled to the end of the body is otherwise reached only by wrapping round the whole dialog.
