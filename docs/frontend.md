...
An AI question no longer opens a modal over whatever you were doing. `PendingQuestionsPanel` is a collapsible section at the top of the selected ticket that pushes the workspace down. It shows a tab strip when more than one model is asking, and the countdown once in its header, since every request in a step shares one clock. For any other ticket, `AIQuestionProvider` renders a slim bar at the top of the app naming the ticket that is waiting; it can be opened or dismissed.
