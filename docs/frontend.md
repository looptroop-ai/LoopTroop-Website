...
A dialog's focus scope is the dialog **plus the popups it owns**. `DropdownPicker` and `ModelPicker` portal their popup to `document.body`, so a picker opened inside a window is a DOM sibling of the whole application; both mark their popup with `data-lt-portal`, and the trap treats every such element as part of the open dialog. Without that, a picker inside a window could be opened from the keyboard and not used, and a picker that moves focus into its own popup would step outside the boundary entirely.

The keyboard-shortcuts overlay renders above every other surface, including the ticket view and the app's modal routes. An overlay that both contains focus and sits underneath what the user can see leaves the visible interface inert and unresponsive.
