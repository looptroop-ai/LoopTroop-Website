...
The folder-ignore cards documented in [LoopTroop Folder Ignore Policy](configuration.md#looptroop-folder-ignore-policy) live in Project **Advanced**, after Git repository validation, and retain the same three-card style and skip warning as the former top-level control. Configuration **Advanced** supplies the future-project default. A hover/focus `?` summary explains `/.looptroop/`, `/.ticket/`, and the three destinations; activating it opens the canonical configuration section. This setting is absent from ticket surfaces because it belongs to the attached project and its worktrees.
