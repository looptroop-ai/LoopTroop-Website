...
A ticket card's title is a real button carrying the card's accessible name, so the board can be worked from the keyboard; clicking anywhere else on the card still opens the ticket. The card itself is deliberately not a control — it holds its own nested controls, which assistive technology cannot expose correctly inside one.
...
Inside a ticket, `Escape` belongs to whatever is open on top of it. The ticket dashboard's handler ignores the key when the event has already been handled, and when it comes from inside a dialog, alert dialog, menu, listbox, combo box, or any Radix popper — so dismissing a nested confirmation no longer also leaves the ticket. `DropdownPicker` portals its popup to `document.body` as a plain element, outside every one of those selectors, so it stops the key itself; it also exposes `aria-expanded`, `aria-haspopup`, and the id of the popup it owns, and returns focus to its trigger when it closes.
