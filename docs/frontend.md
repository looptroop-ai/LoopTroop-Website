...
An **installed** LoopTroop serves this bundle from the daemon, so the interface and the API share one origin on one port and the session cookie minted by `looptroop open` authenticates every request — there is no proxy and no token in the page. The proxy and token-injection behaviour described in this section is the development stack.
...
- the About overlay, which has no route of its own: it sits above the routed modals and is closed by browser Back or Forward, and by closing the Configuration screen it was opened from
...
The navigator width is the deliberate exception because it is browser chrome, not ticket content. `TicketDashboard.tsx` restores it from the browser-local `localStorage` key `looptroop-ticket-nav-width`, clamps it to a minimum of 200 px and a maximum of 50% of the current viewport width, defaults to 280 px, and writes it back only when a drag ends.
...
Execution setup approval exposes the separately published approval copy's workspace inputs, functional workspace probes, and Git-hook handling as first-class plan fields. The user can review and edit each ignored or untracked file or directory before approval. Detected hooks and the ticket-locked project policy are read-only, backend-authoritative evidence; ordered validation commands remain editable, including adding, reordering, changing, or removing every command without an additional waiver gate. Raw or structured edits that attempt to change the policy are normalized back to the locked project value. Regenerate preserves commentary and any unsaved structured/raw baseline durably, then returns the ticket to **Drafting Workspace Setup Plan** for a new version. The runtime/final review surfaces whether input materialization and hook validation executed, failed, or was deliberately skipped.
...
Restored AI and per-model tabs keep model-attributed system milestones, including matching totals and complete-history exports. Model tabs come from all recorded models in the selected phase and attempt, or the whole ticket in Full Log, so filtering or paging cannot hide another model. The catalog survives closing and reopening a log view. Changing ticket or phase attempt resets selection to ALL; an unavailable initial model falls back after discovery or a failed request. Display, history, metrics, and export use the same selected tab.

History refresh after older pages have been loaded starts again from the newest page, updates totals and the model catalog, then sequentially fetches older pages using the fresh cursors. Refresh covers the previously loaded page count, stopping earlier if history ends. New log entries can move page boundaries, so the exact oldest displayed row may change.
...
The browser opens the ticket stream through the same-origin `/api/stream` route, matching normal API fetches and avoiding dev-environment host/CORS drift. In development, that path lets Vite inject backend auth without exposing the token to client code. In an installed build, the browser sends its session cookie. The API does not accept `apiToken` or any other credential in the query string. When the backend emits `replay_gap`, the frontend clears the persisted `lastEventId`, invalidates the current ticket's ticket/artifact/interview/bead/Manual QA/AI-detail/log queries, and keeps the current subscription alive while those reads refill the workspace. If the transport later reconnects, it does so without the stale cursor. At the UI level, `replay_gap` means "reload this ticket from durable backend state and keep streaming" rather than "reload the page." Live OpenCode event translation, streaming upserts, coalescing, finalization, durable JSONL rules, and SSE cadence are unchanged by paginated restoration.
...
Structured log rows may identify the model either through `modelId` or through `source: "model:<full-model-id>"`. The frontend treats that `model:` prefix as a first-class source value: it powers log colouring, per-model tabs, and Current Activity attribution when a separate `modelId` field is absent.
...
An AI question no longer opens a modal over whatever you were doing. `PendingQuestionsPanel` is a collapsible section at the top of the selected ticket that pushes the workspace down. It refreshes that ticket's pending-question list when mounted, so questions that arrived while the ticket was closed appear without waiting for the next aggregate poll. It shows a tab strip when more than one model is asking, and the countdown once in its header, since every request in a step shares one clock. For any other ticket, `AIQuestionProvider` renders a slim bar at the top of the app naming the ticket that is waiting; it can be opened or dismissed.
...
A ticket card's title is a real button carrying the card's accessible name — the visible title first, then the ticket id, then the wait it is under, if any — so the board can be worked from the keyboard and the name matches the words on screen; clicking anywhere else on the card still opens the ticket. The card itself is deliberately not a control — it holds its own nested controls, which assistive technology cannot expose correctly inside one.
...
The Kanban filter slider opens the Triage & Filter Control Bar for project, status, phase, priority, stale/inactive, errors, saved preset, and sort controls. Status is a multi-select of every workflow step grouped by phase; Phase is a multi-select of all ten workflow groups (To Do, Discovery, Interview, Specs (PRD), Blueprint (Beads), Pre-Implementation, Implementation, Post-Implementation, Done, Errors); both narrow tickets within their existing Kanban columns. The Errors control is tri-state: `All states`, `Has errored before` (tickets with `hasPastErrors`, including currently blocked), and `Currently blocked` (only `BLOCKED_ERROR`). The hidden slider button shows a count badge when any non-search triage filter or non-default sort is active. The bar's **Reset** button treats search and hidden mock tickets as filters like any other: it appears whenever either is active, and clears them along with the rest. Saved presets are project-scoped and stored only in the durable UI-state record (`looptroop-ui-state`, under `presetsByProject`), the same channel as filters and theme; committed state is written before the refreshed UI paints, so a saved preset survives an immediate refresh. On load the provider rehydrates from that record and skips writing state back on its first render, so a failed or empty read can never overwrite good data. As a one-time recovery, any legacy standalone `looptroop-presets-*` keys from older builds are read back into UI state at startup; the app no longer writes those keys, and each key it read from is removed once — and only once — the merged record has been written back successfully. Deleting them before that write would destroy the only copy if storage refused it, so a private window or a full quota leaves the legacy keys in place and recovers from them again on the next load; a refused write also leaves them queued, so the next state change that does reach storage retires them rather than letting a later deletion be undone on the following load, and the same applies to a key storage refuses to delete. That write re-reads the durable record and merges the recovered presets into it, rather than into the copy the tab started from: every other write here replaces the record wholesale, which another tab can recover from, but this one is followed by deleting the legacy keys and cannot be undone. Retiring them is what lets the record express a deletion: a preset deleted while those keys still existed was copied back out of them on the next load. Presets expose their full saved details on hover instead of expanding the dropdown row. The default sort is `Last Updated (Newest first)`.
...
### Overlay Semantics And Focus Containment

`CenteredModal`, `FullScreenModal`, and the keyboard-shortcuts overlay are exposed as modal dialogs named by their own heading (`role="dialog"`, `aria-modal="true"`, `aria-labelledby`). The attributes ship together with the behavior they promise, through `useDialogFocus` (`src/hooks/useDialogFocus.ts`): opening one moves focus into the dialog, Tab and Shift+Tab wrap within it, everything outside is marked `inert` and `aria-hidden`, and closing it restores focus to whatever had it before. Inertness is applied by walking to the document root and marking the siblings on the way, so a second overlay opened over a first — About over Configuration — hides the outer one and restores exactly that on close. Dialogs built on the Radix primitive already provide the same guarantees.

A dialog's focus scope is the dialog **plus the popups it owns**. `DropdownPicker` and `ModelPicker` portal their popup to `document.body`, so a picker opened inside a window is a DOM sibling of the whole application. Both mark their popup with `data-lt-portal`, whose value is the id of the element that opened it — ownership, not mere presence. A picker belonging to the window in front is part of that window's tab order; one left open behind a newer overlay is inert like the rest of the page. Without the first, a picker inside a window can be opened from the keyboard and not used; without the second, summoning an overlay over an open picker puts that picker in the overlay's keyboard path.

Opening a `DropdownPicker` moves focus into its popup, and closing it hands focus back to the control that opened it — tab order follows document order, and a popup portaled to the end of the body is otherwise reached only by wrapping round the whole dialog. Both pickers hand focus back however the list closes, not only on Escape: choosing an entry closes the list from inside itself, detaching the element the keyboard was on, which would otherwise leave the document focused on nothing.

Overlays that manage their own focus are left alone. Radix menus and dialogs opened from inside a window bring their own focus scope but still route key events through the React tree, so the surrounding trap stands aside whenever focus is outside its own scope rather than pulling it back to its first control.

The keyboard-shortcuts overlay renders above every other surface, the portaled pickers included (`SHORTCUTS_OVERLAY_Z_INDEX`). An overlay that both contains focus and sits underneath what the user can see leaves the visible interface inert and unresponsive.
