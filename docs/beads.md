...
When an enabled Manual QA round is submitted with any explicit Fail, one locked-main-implementer prompt plans every user-selected failure merge group. It receives focused ticket, approved PRD, existing-bead, final-test, checklist/result, evidence-reference, and diff metadata and must successfully use at least one read-only repository inspection tool. That means a completed `read`, `grep`, `glob`, `list`, `codesearch` or `lsp` call whose file or directory arguments stay inside the ticket worktree; a `read` that names no file, or a call pointing outside the worktree, does not count. A `grep` pattern is search text rather than a location, so searching for something that looks like a path is fine. This lets it discover newly relevant target files rather than copying only the original bead paths. The strict `<MANUAL_QA_FIX_BEADS>` response must provide one candidate per application-owned stable group key with a title, description, PRD refs, implementation context, acceptance criteria, tests, appropriate planned command guidance, labels, dependency inputs, and safe project-relative targets. A candidate may instead use an empty command list with a visible `testCommandReason`.
