...
> [!NOTE]
> In development, not yet released: the daemon checks waiting tickets on startup and roughly every 30 seconds after each sweep, even when no UI is open. It only completes PRs already merged on GitHub; the Merge action still initiates a merge. Failed background checks keep the ticket waiting and retry with delays from one to five minutes. Checks resume after a daemon restart. Shutdown stops new checks and waits for the active check before closing storage, subject to the process shutdown deadline. On restart, a verified merge report resumes completion without repeating remote work. Routine GitHub reads do not block Cancel or Close; actual completion is serialized with those actions. Ticket GET requests no longer contact GitHub or complete merges. Checks use the stored PR number, including after its head branch is deleted. Squash and rebase merges are supported by verifying GitHub’s recorded landed commit on the remote base; the PR head must still match the approved candidate. The Merge action also passes that expected SHA to GitHub, which rejects a head changed before the merge request arrives. Once the finish decision is recorded, cleanup cannot be canceled. Repeating Merge during cleanup or after completion returns success only when the verified merge is already recorded.
