...
PR21 (unreleased) adds `modelIds` to the newest-page response: sorted distinct model IDs across the requested ticket, phase, attempt, and bead scope, independent of `view`, `modelId`, and the page limit. Older pages omit this metadata with the totals. AI history includes attributed system milestones and matches source-only model identity when no explicit model ID exists. Pagination, counts, and exports use the same filter.
