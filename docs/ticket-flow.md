...
- **`CODING` is the versioning exception**: retries use the active bead/checkpoint recovery path instead of creating phase-attempt versions; the reset happens only when safe.
...
warns only while those values differ; restoring the snapshot clears the warning.
Hydration or refetch does not replace values already being edited. Successful
saves acknowledge the submitted snapshot, while failed saves and later edits
keep the newer draft visible. An unsaved modal draft is browser state, not a
reload-persistence guarantee.
...
- `RETRY`: Re-enters `previousStatus`; non-implementation phases archive the failed attempt and create a fresh version first, while `CODING` uses the active bead/checkpoint path and resets only when recovery is safe.
...
Recovery controls are server-advertised. Setup approval does not imply setup
editing or note-bearing retry; those actions appear only for the live blocked
...
For interview coverage, a clean result means **no gaps and no follow-up
...
- **Cancellation** is available across active work and human gates, but it stops being available once final cleanup starts or a verified merge is recorded.
...
- **`WAITING_INTERVIEW_ANSWERS`:** The dashboard pauses for user answers. Questions are presented in adaptive, dynamic batches of 1 to 3 to optimize cognitive load. Skip and "skip all" choices are supported, each with an optional reason that the model filling that answer later gets to read. The active draft shows that autosave is on, reports pending/saving/saved/conflict/failure state, and displays the last acknowledged save as a relative time with the exact local timestamp on hover. When a checkable repository fact affects a next batch, the interview model may inspect only the relevant area read-only; user intent remains a user decision.
- **`VERIFYING_INTERVIEW_COVERAGE`:** The winner checks the answers for ambiguities or gaps, spawning targeted follow-up rounds if budget permits, and may use focused read-only inspection to confirm technical facts rather than guessing them. Clean means no remaining gaps and no follow-up questions; a `0%` budget means no follow-up questions are generated.
- **`WAITING_INTERVIEW_APPROVAL`:** Gatekeeper review. The user approves the structured YAML specs with content-hash protection (`expectedContentSha256` matching check). The editable draft visibly autosaves across reloads and retains its loaded baseline; a missing save baseline returns `428`, a stale one `409`, and a failed save remains unsaved for retry. The user must click **Save** to apply it to the authoritative interview artifact and trigger downstream effects.

### Specs Loop (PRD)
- **`DRAFTING_PRD`:** Models resolve skipped questions into a Full Answers artifact (`answered_by: ai_skip`), reading any reason the user gave for skipping, then draft comprehensive feature requirements. Skip reasons reach this prompt and no other; they are stripped from the interview artifact everywhere else and never appear in a Full Answers artifact. Full Answers and PRD drafting can use focused read-only inspection only when relevant files cannot substantiate a needed repository-specific technical fact.
...
- **`WAITING_PRD_APPROVAL`:** Gatekeeper review of the PRD requirements spec with content-hash matching, supported by the winning Full Answers reference context. The editor visibly autosaves its draft and retains its loaded baseline; a missing save baseline returns `428`, a stale one `409`, and a failed save remains unsaved for retry. **Save** remains required to update the authoritative PRD and apply downstream effects. If unresolved coverage gaps remain, the user can run one manual extra fix at a time before approving or approving with gaps.
...
- **`WAITING_EXECUTION_SETUP_APPROVAL`:** The generated plan is published into a separate approval attempt for user review. It presents the readiness assessment, approved ignored or untracked workspace inputs, required temporary setup steps, ordered workspace probes, detected Git hooks, the read-only project policy frozen at Start, editable explicit hook commands, and version history. The user can review each proposed file or directory, edit the plan, or regenerate it with commentary; raw and structured policy edits are replaced with the backend-owned locked value. The editor visibly autosaves its draft, while **Save** remains required to update the authoritative approval copy and apply its downstream effects. A malformed draft remains reviewable here with Approve/Edit disabled and Regenerate available.
...
- **`CODING`:** The executor processes one bead at a time in dependency order. LoopTroop records the bead's start `HEAD` before publishing it as `in_progress`; if that checkpoint cannot be read, the bead remains pending. The agent gets narrow context, uses planned test commands as adaptable guidance, runs appropriate bead-scoped checks, and must return a structured all-pass completion marker. LoopTroop validates the marker but does not independently rerun the frozen commands. Deadline exhaustion enters the existing Ralph reset/fresh-iteration path when recovery is safe, and local finalization must still succeed. An OpenCode step-cap conflict can refuse that destructive reset while preserving the edited root config and sidecar; a later bead may continue without a fresh cap when no reset is needed, with valid marker evidence excluding that root config from delivery. Failed iterations, user retry guidance, and finalization failures append to three separate structured histories. After every bead completes, backend-executed Final Testing remains the hard ticket-level gate.
...
| `WAITING_EXECUTION_SETUP_APPROVAL` / `PREPARING_EXECUTION_ENV` | edit, regenerate, approve/rewind | These are setup-plan routes, not implied `BLOCKED_ERROR` recovery controls. Approval-phase regeneration enters `GENERATING_EXECUTION_SETUP_PLAN` in a fresh version. During runtime setup, manual editing stops setup and rewinds directly to approval with the current plan; regeneration also archives the setup-plan/runtime attempts, preserves the tool cache, and enters the drafting status before approval is required again. |
| `WAITING_PR_REVIEW` | `merge`, `close_unmerged`, `cancel` | Review resolution decides whether the ticket exits with a merged PR or a closed unmerged branch; Close Without Merge never merges. Initial remote-refresh failures leave the ticket waiting with a recovery receipt. Once a same-PR decision is recorded, its completion resumes through the matching Merge path for a verified merge or through background recovery for a closed-unmerged decision; conflicting Merge, Close Without Merge, and Cancel actions are rejected. |
...
| `BLOCKED_ERROR` | `retry`, optional retry with extra note, optional edit setup plan, optional `continue`, `cancel` | The server-advertised action list is authoritative. **Retry with extra note...** appears for a live error from `CODING` or `PREPARING_EXECUTION_ENV` when advertised and sits beside **Retry**. Coding starts its existing fresh-bead retry. Setup sends only the note to the preserved session and grants one manual attempt beyond the automatic budget. **Edit setup plan...** appears only for setup when advertised and opens a confirmation dialog before rewinding to approval. `continue` appears only when a preserved OpenCode session is still live. Final-test local-only files are resolved automatically and expose no blocked recovery action. |
| `CLEANING_ENV` | none | The finish decision is already recorded. Cleanup cannot be canceled, and restarting the daemon retries interrupted cleanup. |
...
- **`CODING` Exception:** `CODING` does not create new phase attempts. It runs a bead-scoped recovery loop: when safe, it resets the active bead's worktree back to its recorded `beadStartCommit` snapshot and schedules it again; a recovery conflict can refuse the destructive reset and leave the ticket blocked.
- **Optional user guidance:** From a live implementation or execution-setup block, the server may advertise **Retry with extra note...**. It opens a dialog and requires a non-blank note of at most 20,000 characters. Coding recovery first proves that the same failed or paused bead can be safely reset, then appends a structured `userRetryNotes` entry and starts the existing fresh-bead retry. Execution setup sends only the note to the preserved OpenCode session and allows one manual attempt beyond the automatic budget. It does not archive the runtime phase attempt or save the note as future setup context. Setup approval does not imply this action. If recovery fails, the ticket stays blocked.
...
- `CODING` is the exception: recovery stays bead-scoped (`bead_execution:*` artifacts, checkpoint finalization, and a safe worktree reset when available) rather than creating phase-attempt versions.
...
| **Pull-request completion is interrupted** | The persisted merge or close decision resumes after revalidation for the same PR. A remote refresh failure leaves `WAITING_PR_REVIEW` with a durable recovery receipt and no decision; an expired native log cursor is retried once by the client before remaining visible as an error. |
...
| **Recovery & History** | Archived phase attempts, `final_test_file_effects_audit`, continuation candidates, same-PR merge/close decisions, typed pull-request refresh receipts, and read-only prior planning generations addressable by `phaseAttempt`. |
